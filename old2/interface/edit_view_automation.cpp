...
//
//
#include "edit_view_automation.h"
...
void EditViewAutomation::paint_value_text(Painter &p,int p_x,int p_y,float p_val) {
	
	
	String s=automation->get_property()->get_text_value( automation->get_property()->get_value_from_coeff(p_val) ) ;

	FontID f = font( FONT_AUTOMATION_VALUE );
	
	int w=p.get_font_string_width(f,s);
	int h=p.get_font_height(f);
	...
	y+=p.get_font_ascent(f);
	

	p.draw_text(f,Point(x+2,y+2),s,color(COLOR_AUTOMATION_FONT_SHADOW));
	p.draw_text(f,Point(x-2,y-2),s,color(COLOR_AUTOMATION_FONT_SHADOW));
	p.draw_text(f,Point(x,y),s,color(COLOR_AUTOMATION_FONT));

	
}

void EditViewAutomation::paint_row_lines(Painter &p,int p_from_row, int p_to_row) {
	...
	int margin_w=constant(C_AUTOMATION_MARGIN_WIDTH);
	...
	for (int i=p_from_row;i<=p_to_row;i++) {
		...
		if ( (tick % TICKS_PER_BEAT)==0 ) {//beat
			...
			if (paint)
				p.draw_fill_rect(Point(from_x,i*row_size),Size(width_x,1),color(COLOR_AUTOMATION_BEAT_LINE));
		
			if (editor->get_song()->get_bar_map().get_bar_beat(tick/TICKS_PER_BEAT)==0)  {
				p.draw_fill_rect(Point(from_x,i*row_size),Size(width_x,1),color(COLOR_AUTOMATION_BAR_LINE));
			}
			
		} else
			p.draw_fill_rect(Point(from_x,i*row_size),Size(width_x,1),color(COLOR_AUTOMATION_SUBBEAT_LINE));

	}

}

void EditViewAutomation::paint_frames(Painter& p,int p_from_row,int p_to_row) {
	...
	const StyleBox &sb=stylebox(SB_AUTOMATION);
	const StyleBox &sb_shared=stylebox(SB_AUTOMATION_SHARED);
...
void EditViewAutomation::paint_envelopes(Painter &p,int p_from_row, int p_to_row) {
	...
	int margin_width=constant(C_AUTOMATION_MARGIN_WIDTH);
	...
	Color env_col=color(COLOR_AUTOMATION_ENVELOPE);
	Color env_col_normal=color(COLOR_AUTOMATION_ENVELOPE);
	Color env_col_repeat=color(COLOR_AUTOMATION_ENVELOPE_REPEAT);
	//p.setClipping(true);
	//p.setClipRect ( ,4,size.width-8,size.height-8);
	int prev=-1;
	bool last_is_repeat;
	...
	for (int i=0;i<lines;i++) { //paint an extra one, even if not editable
		...
		int x_ofs=margin_width+(int)(val*(float)(size.width-margin_width*2));
		...
		if (x_ofs<prev)		
			p.draw_fill_rect(Point(x_ofs,line),Size((prev-x_ofs)+2,1),env_col);
		else
			p.draw_fill_rect(Point(prev,line),Size((x_ofs-prev)+2,1),env_col);
		//p.drawLine(prev,line-1,x_ofs,line);
	
		prev=x_ofs;
	}
	
	//p.setRenderHint(Painter::Antialiasing,false);
	//p.setClipping(false);
...
void EditViewAutomation::paint_selection(Painter&p,int p_clip_from,int p_clip_to) {
	...
	int sel_margin=constant(C_AUTOMATION_SELECTION_MARGIN);
	...
	if ( editor->get_selection_begin_blocklist() <= current_bl && editor->get_selection_end_blocklist() >= current_bl &&
		...
		p.draw_fill_rect(Point(sel_margin,from_y),Size(size.width-sel_margin*2,to_y-from_y),color(COLOR_AUTOMATION_SELECTION));
	}
		    
	
}

void EditViewAutomation::fix_pre_scroll(int p_scroll) {
	
	
}

void EditViewAutomation::draw(const Point& p_pos,const Size& p_size,const Rect& p_exposed) {
	...
	if (get_current_line()!=editor->get_cursor().get_window_offset()) {
		...
		if (row_size*abs(diff) < (size.height/2) ) { //only scroll if it's worth doing itEditViewAutomation( 
	...
	*/
	...
	int row_from=p_exposed.pos.y/row_size;
	if (row_from>0)
		row_from--; //permits proper update on screen-screen copying
	int row_to=(p_exposed.pos.y+p_exposed.size.height)/row_size+1;

	p.draw_fill_rect(Point(0,row_size*row_from),Size(size.width,(row_to-row_from)*row_size),color(COLOR_AUTOMATION_BG));
...
bool EditViewAutomation::find_closest_point(int p_x,int p_y,int p_radius, int *r_block, int *r_point) {
	...
	for (int i=from_block;i<=to_block;i++) {
		...
		for (int j=0;j<d->get_stream_size();j++) {
			...
			int x_ofs=margin_w+lrintf(val*(float)(size.width-margin_w*2));
...
bool EditViewAutomation::screen_to_tick_and_val(int p_x,int p_y,Tick *p_tick, float *p_val) {
	...
	*p_tick+=tick_begin;
	...
	int margin_w=constant(C_AUTOMATION_MARGIN_WIDTH);
	int range=size.width-margin_w*2;
	
	*p_val=(float)(p_x-margin_w)/(float)range;

	return false;
	
}

void EditViewAutomation::compute_point_over(int p_x,int p_y) {
...
void EditViewAutomation::mouse_motion(const Point& p_pos, const Point& p_rel, int p_button_mod_mask) {
	Point delta=p_pos-moving_point.prev_pointer_pos;
	moving_point.prev_pointer_pos=p_pos;
	...
	float new_val;
	float new_lfo;
	if (screen_to_tick_and_val(p_pos.x,p_pos.y,&new_tick,&new_val))
		return;
	...
	if (moving_point.moving_first) {
		...
		if ( abs(delta.x) > abs(delta.y) && p_button_mod_mask&KEY_MASK_SHIFT )
	...
	if (!moving_point.lfo_depthing) {
		...
		moving_point.snap=p_button_mod_mask&KEY_MASK_SHIFT;
...
void EditViewAutomation::cancel_motion() {
...
void EditViewAutomation::get_pos_at_pointer(Point p_pointer, int *p_blocklist,int *p_column, int *p_row) {
	
	*p_row=p_pointer.y/get_row_height()+editor->get_cursor().get_window_offset();
	*p_blocklist=editor->find_blocklist( automation );
	*p_column=0;
}

void EditViewAutomation::mouse_button(const Point& p_pos, int p_button,bool p_press,int p_modifier_mask) {
	...
	if (p_press) {
		...
		if (p_button==BUTTON_LEFT) {
			...
			int row=p_pos.y/get_row_height();
			...
			if (automation->get_block_idx_at_pos(click_tick)<0) { 		
				get_focus=true;
			} else if (p_pos.x<margin_w || (size.width-p_pos.x)<margin_w) {
				get_focus=true;
			}
			if (get_focus) {
		...
		if (p_button==BUTTON_RIGHT && moving_point.moving ) {
		
			/* Cancel motion */
			cancel_motion();
			return;
		} else if ( !moving_point.moving && ((p_button==BUTTON_LEFT && p_modifier_mask&KEY_MASK_CTRL) || p_button==BUTTON_RIGHT)) {
			/* delete point */
			
			int closest_block,closest_point;
		
			if (find_closest_point(p_pos.x,p_pos.y,CLOSEST_POINT_RADIUS, &closest_block, &closest_point)) {
		...
		if (find_closest_point(p_pos.x,p_pos.y,CLOSEST_POINT_RADIUS, &closest_block, &closest_point)) {
			...
			if (p_modifier_mask&KEY_MASK_SHIFT) { //snap!
...
bool EditViewAutomation::can_scroll() {
	
	return !has_focus();
}


bool EditViewAutomation::key(unsigned long p_unicode, unsigned long p_scan_code,bool p_press,bool p_repeat,int p_modifier_mask) {
...
Size EditViewAutomation::get_minimum_size_internal() {
	...
	switch(automation->get_display_size()) {
		...
		case Automation::DISPLAY_SIZE_SMALL: {
			...
			return Size(constant(C_AUTOMATION_SIZE_SMALL),0);
		} break;
		case Automation::DISPLAY_SIZE_MEDIUM: {
			
			return Size(constant(C_AUTOMATION_SIZE_MEDIUM),0);
			
		} break;
		case Automation::DISPLAY_SIZE_BIG: {
			
			return Size(constant(C_AUTOMATION_SIZE_BIG),0);			
		} break;
	}
...
EditViewAutomation::EditViewAutomation(Editor *p_editor, Automation *p_automation) : EditViewBlockList(p_editor) {
...
void EditViewAutomation::show_popup() {
	
	/*
		
	Tick tick;
	float val;
	Point e=mapFromGlobal(QCursor::pos());
...
EditViewAutomation::~EditViewAutomation()
{
}
