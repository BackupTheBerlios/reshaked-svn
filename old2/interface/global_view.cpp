...
int GlobalView::get_block_list_skinbox(BlockList *p_block,bool p_selected) {
	
	if (dynamic_cast<Automation*>(p_block))
		return p_selected?SB_GLOBAL_VIEW_AUTOMATION_SELECTED:SB_GLOBAL_VIEW_AUTOMATION;
	else if (dynamic_cast<Track_Pattern*>(p_block))
		
		return p_selected?SB_GLOBAL_VIEW_PATTERN_SELECTED:SB_GLOBAL_VIEW_PATTERN;
	else //audio in a future
		return p_selected?SB_GLOBAL_VIEW_PATTERN_SELECTED:SB_GLOBAL_VIEW_PATTERN;
...
void GlobalView::resize_check_consistency() {
	...
	if (get_pixel_h_offset()>(get_total_pixel_width()-get_width())) {
		int new_ofs=get_total_pixel_width()-get_width();
		if (new_ofs>0)
...
void GlobalView::resize(const Size& p_new_size) {
...
void GlobalView::mouse_motion(const Point& p_pos, const Point& p_rel, int p_button_mask) {

	if (p_button_mask&BUTTON_LEFT) { //draggingGlobalView(
		...
		if (moving_block.moving==false && resizing_block.resizing==false) {
			...
			switch (mouse_data.action_at_selecting) {
				...
				case MouseData::POS_OVER: {
					...
					if ((edit_mode==EDIT_MODE_SELECT && p_button_mask&KEY_MASK_SHIFT) || edit_mode!=EDIT_MODE_SELECT )  { //copy

						compute_moving_block_list();
						if ((edit_mode==EDIT_MODE_SELECT && p_button_mask&KEY_MASK_CTRL) || edit_mode==EDIT_MODE_COPY_LINK_BLOCK)
		...
		if (moving_block.moving) {
			...
			moving_block.current_x=(float)p_pos.x/display.zoom_width+display.ofs_x;
			moving_block.current_y=(float)p_pos.y/display.zoom_height+display.ofs_y;
		...
		if (resizing_block.resizing)
			resizing_block.current_y=(float)p_pos.y/display.zoom_height+display.ofs_y;

		if (p_pos.y>get_height()) {
			display.ofs_y+=(float)(p_pos.y-get_height())/display.zoom_height;
			get_painter()->set_cursor_pos( get_global_pos()+Point( p_pos.x, get_height() ) );
		}
		if (p_pos.y<0) {
			display.ofs_y+=p_pos.y/display.zoom_height;
			if (display.ofs_y<0)
				display.ofs_y=0;
			get_painter()->set_cursor_pos( get_global_pos()+Point( p_pos.x, 0 ) );
		...
		if (get_screen_to_blocklist_and_tick( p_pos.x, p_pos.y,&blocklist,&tick))
			return;
...
void GlobalView::mouse_button(const Point& p_pos, int p_button,bool p_press,int p_modifier_mask) {
	
	if (p_press) {
	
		if (p_button==BUTTON_RIGHT) { //cancel any moving stuffGlobalView(
		...
		mouse_data.click_x_from=(float)p_pos.x/display.zoom_width+display.ofs_x;
		mouse_data.click_y_from=(float)p_pos.y/display.zoom_height+display.ofs_y;
	
		mouse_data.clicked_list=-1;
		mouse_data.clicked_block=-1;
		int blocklist=-1,block=-1;
	
		if ( edit_mode==EDIT_MODE_ADD_BLOCK || (edit_mode==EDIT_MODE_SELECT && p_modifier_mask&KEY_MASK_CTRL && !(p_modifier_mask&KEY_MASK_SHIFT))) {
	
			//printf("one\n");
			if (!get_click_location(p_pos.x,p_pos.y,&blocklist,&block))
		...
		if (edit_mode==EDIT_MODE_SELECT) {
			if (is_block_selected(blocklist,block)) {
				...
				mouse_data.shift_when_deselecting=p_modifier_mask&KEY_MASK_SHIFT;
			} else {
		
				if (! (p_modifier_mask&KEY_MASK_SHIFT))
					clear_selection();
		...
		if (get_screen_to_blocklist_and_tick( p_pos.x, p_pos.y,&ablocklist,&atick))
			return;
...
void GlobalView::paint_name(Painter&p, int p_blocklist,int p_ofs) {
	...
	if (dynamic_cast<Track*>(b)) {
		...
		name=a->get_property()->get_caption();
		des_width=0.35;
	} else return; // ??
...
void GlobalView::paint_block(Painter& p,int p_x,int p_y,int p_list,int p_block,bool p_drawover,bool p_notpossible,Tick p_len_othersize,bool p_no_contents,bool p_is_repeat) {
	...
	StyleBox sb;
	if (p_is_repeat) 
		sb=stylebox( SB_GLOBAL_VIEW_REPEAT);
	else if (is_block_selected(p_list,p_block))
		sb=stylebox(get_block_list_skinbox( blocklist, true ));
	else
	...
	if (!p_no_contents) {
		...
		GlobalViewBlocks::paint_block_contents( p, p_x,p_y, f_width, c_height, blocklist->get_block(p_block) ,is_block_selected(p_list,p_block),(int)f_height,color ( COLOR_GLOBAL_VIEW_AUTOMATION ),color ( COLOR_GLOBAL_VIEW_PATTERN_NOTE));
...
void GlobalView::draw(const Point& p_pos,const Size& p_size,const Rect& p_exposed) {
	...
	int beat_to=get_beat_at_pixel(get_height());
	
	//FointID pixmap_font=font(FONT_GLOBAL_VIEW);
	...
	for (int i=beat_from;i<=beat_to;i++) {
		...
		if (song->get_bar_map().get_bar_beat( i )) { //regular beat
			...
			if (get_beat_pixel_size()<3) //dont paint it so small, make this constant later
				continue;
			p.draw_fill_rect(Point(0,line),Size(get_width(),1),color(COLOR_GLOBAL_VIEW_BEAT_LINE));
		} else { //bar 
			p.draw_fill_rect(Point(0,line),Size(get_width(),1),color(COLOR_GLOBAL_VIEW_BAR_LINE));
		}
	}
	...
	for (int i=0;i<blocklist_count;i++) {
		...
		p.draw_fill_rect( Point( f_from_x+f_width,0 ), Size( 1, get_height() ), color( COLOR_GLOBAL_VIEW_SEPARATOR ) ); // 111,111,111
...
void GlobalView::block_repeat_set_slot() {
...
void GlobalView::block_repeat_unset_slot() {
...
GlobalView::GlobalView(Editor *p_editor) 
{
	// TODO: put constructor code here

	set_bg_on_updates(false); //speedup
