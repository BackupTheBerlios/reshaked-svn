...
GUI::BitmapID PixmapData::create_bitmap_from_buffer(const unsigned char *p_png_buffer) {
				...
				default:  // 24 bits (RGB) case 8:
					for (int x=0;x<width;x++) {
						for (int y=0;y<height;y++) {
							...
							painter->set_bitmap_pixel(bmp,GUI::Point(x,y),GUI::Color(pixel[0],pixel[1],pixel[2]));
						}
					}
				break;
			}
				...
				default: // 32 bits (RGBA) case 8:
					...
					for (int x=0;x<width;x++) {
						for (int y=0;y<height;y++) {
							...
							painter->set_bitmap_pixel(bmp,GUI::Point(x,y),GUI::Color(pixel[0],pixel[1],pixel[2]),pixel[3]);
						}
					}
...
GUI::BitmapID PixmapData::pixmap_id_list[PIXMAP_MAX];

GUI::BitmapID PixmapData::get_pixmap(PixmapDataList p_pixmap) {
...
void PixmapData::set_painter(GUI::Painter *p_painter) {
