...
void GlobalViewBlocks::paint_block_contents(Painter &p, int p_x,int p_y,int p_w, int p_h, BlockList::Block *p_block,bool p_selected,int p_clip_to_len,Color p_auto_color,Color p_note_color) {
	...
	if (dynamic_cast<Automation::AutomationBlock*>(p_block)) {
		...
		for (int i=0;i<p_h;i++) {
			...
			p.draw_fill_rect( Point( p_x+prev_x,p_y+v_pixel ), Size( v_pixel-prev_x,1 ), p_auto_color );
		...
		for (int i=0;i<note_list.size();i++) {
			...
			p.draw_fill_rect( Point( note_x,p_y+pos ), Size( 1,len), p_note_color );
