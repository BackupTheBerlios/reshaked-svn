...
class GlobalView : public Widget {
...
private:
	...
	//Color get_block_list_color(BlockList *p_block);
	int get_block_list_skinbox(BlockList *p_block,bool p_selected=false);
	...
	void paint_name(Painter&p, int p_blocklist,int p_ofs);
		
	void paint_block(Painter& p,int p_x,int p_y,int p_list,int p_block,bool p_drawover=false,bool p_notpossible=false,Tick p_len_othersize=-1,bool p_no_contents=false,bool p_is_repeat=false);
