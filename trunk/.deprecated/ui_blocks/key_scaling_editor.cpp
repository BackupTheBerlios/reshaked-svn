//
// C++ Implementation: key_scaling_editor
//
// Description: 
//
//
...
void KeyScalingEditor::paintEvent(QPaintEvent *e) {
	...
	for (int i=0;i<width();i++) {
		...
		int h=(int)( get_volume_scale( (float)i/(float)width() , val ) * height() );
...
void KeyScalingEditor::mousePressEvent(QMouseEvent *e) {
...
void KeyScalingEditor::mouseMoveEvent(QMouseEvent *e) {
...
void KeyScalingEditor::mouseReleaseEvent(QMouseEvent *e) {
...
void KeyScalingEditor::changed() {
	
	update();
}

KeyScalingEditor::KeyScalingEditor(QWidget *p_parent,const Skin &p_skin) : QWidget(p_parent) {
...
KeyScalingEditor::~KeyScalingEditor()
{
}
